...
public class LeaveRequestService : ILeaveRequestService
{
    ...
    public async Task<List<LeaveRequestDTO>> GetLeaveRequestsByFilter(LeaveRequestFilterDTO filterDTO)
    {
        ...
        if (filterDTO.EmployeeId.HasValue) query = query.Where(lr => lr.EmployeeId == filterDTO.EmployeeId);
        ...
        else if (filterDTO.EndDate.HasValue) query = query.Where(lr => lr.EndDate <= filterDTO.EndDate.Value);


        if (!string.IsNullOrEmpty(filterDTO.Status)) query = query.Where(lr => lr.Status == filterDTO.Status);

        if (filterDTO.RequestNumber.HasValue) query = query.Where(lr => lr.Id == filterDTO.RequestNumber.Value);
        ...
        var leaveRequests = await query
            .Select(lr => new LeaveRequestDTO
            {
                ...
                Comment = lr.Comment,
                Status = lr.Status,
                ApprovalRequestId = lr.ApprovalRequest != null ? lr.ApprovalRequest.Id : null,
    ...
    public async Task<LeaveRequestDTO> GetLeaveRequestById(int id)
    {
        ...
        return new LeaveRequestDTO
        {
            ...
            ApprovalRequestId = leaveRequest.ApprovalRequest != null ? leaveRequest.ApprovalRequest.Id : null,
