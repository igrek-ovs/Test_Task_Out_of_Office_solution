...
public class ProjectService : IProjectService
{
    ...
    public async Task<List<ProjectDTO>> GetProjectsByFilter(ProjectFilterDTO filterDTO)
    {
        ...
        else if (filterDTO.StartDateTo.HasValue) query = query.Where(lr => lr.EndDate <= filterDTO.StartDateTo.Value);

        if (filterDTO.Status.HasValue) query = query.Where(p => p.Status == filterDTO.Status.Value);

        if (filterDTO.ProjectNumber.HasValue) query = query.Where(p => p.Id == filterDTO.ProjectNumber.Value);
            ...
            switch (filterDTO.SortBy)
            {
                case "name":
                    query = filterDTO.SortAscending == true
                        ? query.OrderBy(p => p.ProjectType)
                        : query.OrderByDescending(p => p.ProjectType);
                    break;
                case "startDate":
                    query = filterDTO.SortAscending == true
